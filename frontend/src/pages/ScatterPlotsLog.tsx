...
function ScatterPlotsLog() {
  ...
  let yScale: d3.ScaleLogarithmic<number, number, never>;
  let xTicks;
  let yTicks;
  if (dimensions && chronicData.length > 0) {
    const xDomain = d3.extent(chronicData, (d) => new Date(d.date)) as Date[];
    const yDomain = d3.extent(chronicData, (d) => d.items.length) as number[];
  ...
  return (
    <div className="w-full bg-accent-1-500 rounded p-8 pt-10">
      <h1 className="text-4xl font-medium capitalize">Logarithmic Frequency of Bulletins over Time:</h1>
      <div
      ref={wrapperRef}
      className="h-[400px] w-full bg-inherit rounded p-4"
    >
...
export default ScatterPlotsLog;
